...
// ─── LOCAL VIEW MODEL (unchanged from original) ───────────────────────────
class LifeSphere {
  final String id, name, emoji;
  final Color color;
  double score; // 0.0 - 10.0

  LifeSphere({required this.id, required this.name, required this.emoji,
    required this.color, this.score = 5.0});
}

// ─── SCREEN ───────────────────────────────────────────────────────────────
class LifeBalanceScreen extends ConsumerStatefulWidget {
  const LifeBalanceScreen({super.key});
  @override ConsumerState<LifeBalanceScreen> createState() => _LifeBalanceScreenState();
}
...
class _LifeBalanceScreenState extends ConsumerState<LifeBalanceScreen>
  ...
  final List<LifeSphere> _spheres = [
    LifeSphere(id:'health',    name:'Health',     emoji:'💪', color: const Color(0xFF00C97B), score: 7.5),
    LifeSphere(id:'career',    name:'Career',     emoji:'💼', color: const Color(0xFF4D9FFF), score: 6.0),
    LifeSphere(id:'finance',   name:'Finance',    emoji:'💰', color: const Color(0xFFFFD700), score: 4.5),
    LifeSphere(id:'relations', name:'Relations',  emoji:'❤️', color: const Color(0xFFFF6B9D), score: 8.0),
    LifeSphere(id:'learning',  name:'Learning',   emoji:'📚', color: const Color(0xFFBF7FF5), score: 7.0),
    LifeSphere(id:'mindset',   name:'Mindset',    emoji:'🧘', color: const Color(0xFFFF8C69), score: 5.5),
    LifeSphere(id:'social',    name:'Social',     emoji:'🤝', color: const Color(0xFF00D4D4), score: 6.5),
    LifeSphere(id:'fun',       name:'Fun',        emoji:'🎉', color: const Color(0xFFFF6B6B), score: 3.0),
  ];
  ...
  // Load latest snapshot scores into sliders (called once)
  void _loadLatestIfNeeded(LifeBalanceSnapshot? latest) {
    ...
    setState(() {
      for (final s in _spheres) {
        if (latest.scores.containsKey(s.id)) {
          s.score = latest.scores[s.id]!;
        }
      }
    });
  }
  ...
  void _saveSnapshot() {
    ...
    final scores = Map.fromEntries(_spheres.map((s) => MapEntry(s.id, s.score)));
    ref.read(lifeBalanceActionsProvider.notifier).saveSnapshot(scores);
  ...
  Widget build(BuildContext context) {
    final snapshots = ref.watch(lifeBalanceProvider).valueOrNull ?? [];
    final latest = ref.watch(latestBalanceProvider);

    // Load latest scores into sliders once
    _loadLatestIfNeeded(latest);

    // Convert Firestore snapshots to local BalanceSnapshot for history tab
    final history = snapshots.map((snap) => _BalanceSnapshot(
      date: snap.date,
...
// ─── LOCAL SNAPSHOT MODEL (for history display only) ─────────────────────
...
class _AdjustTab extends StatelessWidget {
  ...
  const _AdjustTab({required this.spheres, required this.overallScore,
    required this.balanceLabel, required this.onChanged, required this.onSave});
      ...
      children: [
        ...
        Container(
          ...
          decoration: BoxDecoration(
            ...
            boxShadow: [BoxShadow(color: AColors.primary.withValues(alpha: 0.3),
                blurRadius: 16, offset: const Offset(0,4))],
          ),
          child: Row(children: [
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              const Text('Overall Balance', style: TextStyle(color: Colors.white70, fontSize: 13, fontWeight: FontWeight.w600)),
            ...
            Container(
              ...
              child: Center(child: Text(overallScore.toStringAsFixed(1),
        ...
        GestureDetector(
          ...
          child: Container(
            ...
            child: const Row(mainAxisAlignment: MainAxisAlignment.center, children: [
              ...
              SizedBox(width: 8),
              Text('Save Snapshot', style: TextStyle(color: Colors.white,
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int i = 0; i < n; i++) {
      ...
      canvas.drawCircle(Offset(dotX, dotY), 5,
          Paint()..color = spheres[i].color..style = PaintingStyle.fill);
      canvas.drawCircle(Offset(dotX, dotY), 5,
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(children: [
            ...
            AnimatedContainer(
              ...
              child: Text('${sphere.score.toStringAsFixed(1)} · $_scoreLabel',
          ...
          Row(mainAxisAlignment: MainAxisAlignment.spaceBetween,
              children: ['0', '2.5', '5', '7.5', '10'].map((l) =>
                  Text(l, style: AText.bodySmall)).toList()),
        ]),
      ),
    );
  }
  ...
  Widget build(BuildContext context) {
    if (history.isEmpty) {
      return Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
        ...
        const SizedBox(height: 16),
        const Text('No snapshots yet', style: AText.titleMedium),
        const SizedBox(height: 6),
        const Text('Save your first balance check!', style: AText.bodyMedium),
      ]));
    }
    ...
    return ListView(
      ...
      physics: const BouncingScrollPhysics(),
      children: [
        const Text('Trends (last 8 weeks)', style: AText.titleSmall),
...
class _TrendRow extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final scores = history.map((h) => h.scores[sphere.id] ?? 5.0).toList();
    ...
    return Padding(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
            color: AColors.bgCard, borderRadius: ARadius.lg,
            border: Border.all(color: AColors.border)),
        child: Row(children: [
          ...
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Row(children: [
              ...
              Icon(
                trend > 0.1 ? Icons.arrow_upward_rounded
                    : trend < -0.1 ? Icons.arrow_downward_rounded
                    : Icons.remove_rounded,
                size: 14,
                color: trend > 0.1 ? AColors.primary
                    : trend < -0.1 ? AColors.error
                    : AColors.textMuted,
              ),
            ]),
            const SizedBox(height: 6),
...
class _SnapshotCard extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final avg = snap.scores.values.fold(0.0, (a, b) => a + b) / snap.scores.length;
    ...
    return Padding(
      ...
      child: Container(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(children: [
            Text(isToday ? 'Today' : DateFormat('EEE, MMM d').format(snap.date),
                style: AText.titleSmall),
            const Spacer(),
            Container(
                padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 4),
                decoration: BoxDecoration(
                    color: AColors.primaryGlow, borderRadius: ARadius.full),
                child: Text('Avg ${avg.toStringAsFixed(1)}',
                    style: const TextStyle(color: AColors.primary, fontSize: 12,
          ...
          Wrap(spacing: 8, runSpacing: 8,
              children: spheres.map((s) {
                ...
                return Container(
                  ...
                  decoration: BoxDecoration(
                      color: s.color.withValues(alpha: 0.1), borderRadius: ARadius.full),
                  child: Text('${s.emoji} ${score.toStringAsFixed(1)}',
